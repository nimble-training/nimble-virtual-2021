...
model_basic <- nimbleModel(code, data = list(y = y), constants = list(n = n),
...
abline(v = quantile(y, .05), col = 'red')
...
ppSampler <- nimbleFunction(
          setup = function(model, mcmc) {
                dataNodes <- model$getNodeNames(model, dataOnly = TRUE)
...
n <- nrow(faithful)
y <- faithful$waiting
model_marg <- nimbleModel(margCode, data = list(y = y), constants = list(n = n),
...
n <- nrow(faithful)
y <- faithful$waiting
model_full <- nimbleModel(fullCode, data = list(y = y), constants = list(n = n),
                           inits = list(p = 0.5, mu = mean(y) + c(-1,1)*sd(y), sigma = rep(sd(y)/2, 2),
...
cmcmc_full$mvSamples$getVarNames()
colnames(paramSamples)

latentSampler_full <- latentSampler(model_full, cmcmc_full$mvSamples$getVarNames(), 'xi')
...
latentSampler2_full <- latentSampler2(model_full, mcmc_full$mvSamples, 'xi')
